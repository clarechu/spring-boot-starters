package cn.com.siss.spring.boot.redis.lock.impl;

import cn.com.siss.spring.boot.redis.lock.RedisDistributionLock;
...
public class RedisLockImpl implements RedisDistributionLock {
     ...
     */
    @Override
    public synchronized long lock(RedisTemplate redisTemplate, String lockKey, String threadName) {
        log.info(threadName+"开始执行加锁");
        while (true){ //循环获取锁
            //锁时间
            Long lock_timeout = currtTimeForRedis(redisTemplate)+ LOCK_TIMEOUT +1;
            ...
            if (object.equals("true")){
                ...
                if (currt_lock_timeout_str != null && currt_lock_timeout_str < System.currentTimeMillis()){
                    ...
                    Long old_lock_timeout_Str = Long.valueOf(redisTemplate.opsForValue().getAndSet(lockKey, lock_timeout.toString()).toString());
     ...
     */
    @Override
    public synchronized void unlock(RedisTemplate redisTemplate, String lockKey, long lockValue, String threadName) {
        log.info(threadName + "执行解锁==========");//正常直接删除 如果异常关闭判断加锁会判断过期时间
        //获取redis中设置的时间
        String result = redisTemplate.opsForValue().get(lockKey).toString();
     ...
     */
    @Override
    public long currtTimeForRedis(RedisTemplate redisTemplate){
         Object object = redisTemplate.execute(new RedisCallback<Long>() {
            @Override
    ...
    public Boolean doInRedis(RedisTemplate redisTemplate , RedisConnection redisConnection , Long lock_timeout , String lockKey) throws DataAccessException {
        //定义序列化方式
        RedisSerializer<String> serializer = redisTemplate.getStringSerializer();
        byte[] value = serializer.serialize(lock_timeout.toString());
        boolean flag = redisConnection.setNX(lockKey.getBytes(), value);
